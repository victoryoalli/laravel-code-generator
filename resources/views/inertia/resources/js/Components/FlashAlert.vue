<template>
  <div class="px-6 py-8">
    <div v-if="$page.props.flash.success && show" class="flex items-center justify-between w-full mb-8 bg-green-500 rounded">
    ...
    <div v-if="$page.props.flash.warning && show" class="flex items-center justify-between w-full mb-8 bg-yellow-500 rounded">
      <div class="flex items-center">
        <svg class="flex-shrink-0 w-4 h-4 ml-4 mr-2 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><polygon points="0 11 2 9 7 14 18 3 20 5 7 18" /></svg>
        <div class="py-4 text-sm font-medium text-white">{{ $page.props.flash.warning }}</div>
      </div>
      <button type="button" class="p-2 mr-2 group" @click="show = false">
    ...
    <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="flex items-center justify-between mb-8 bg-red-400 rounded">
      <div class="flex items-center">
