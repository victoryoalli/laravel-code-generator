        ...
        <div class="col-span-2 bg-white shadow rounded-lg overflow-hidden">
            <form action="{{code()->doubleCurlyOpen()}}route('{{str($model->name)->slug()->plural()}}.store'){{code()->doubleCurlyClose()}}" method="POST" novalidate>
                <div class="space-y-3 p-4">
                    ...
                    <div class="">
                        <label class="block text-sm font-medium text-gray-700" for="{{$rel->local_key}}">{{str($rel->name)->human()->title()}}</label>
                ...
                <div class="bg-gray-100 flex items-center justify-end px-4 py-3 space-x-3">
                    ...
                    <button class="px-3 py-1.5 border-lg bg-blue-500 text-blue-50 font-medium rounded">Create</button>
                </div>
            </form>
        </div>
    </div>
